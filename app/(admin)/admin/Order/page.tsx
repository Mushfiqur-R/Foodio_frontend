...
// import OrderDetailsModal from '@/components/OrderDetailsModel'; // ✅ Import modal
...
//       const config = getAuthConfig();
//       await axios.patch(
//         `http://localhost:3000/admin/order/status/${orderId}`,
//         {
...
//     <div className="p-8">
//       {/* Header */}
//       <h1 className="text-[32px] font-semibold text-[#1A3C34] mb-8">Order Management</h1>
...
//           {orders.length === 0 ? (
//             <div className="py-12 text-center text-[#7A7A7A]">No orders found.</div>
//           ) : (
...
//               <tbody>
//                 {orders.map((order) => (
//                   <tr key={order.id} className="border-b border-[#E6E2D8] hover:bg-[#F9F9F9] transition">
...
//                         className={`px-3 py-1 text-xs font-medium rounded-full border cursor-pointer ${getStatusColor(
//                           order.status
//                         )}`}
//                       >
...
//                       <Button
//                         text="Details"
//                         onClick={() => handleDetailsClick(order.id)} // ✅ Open details modal
//                         height=""
//                         width="auto"
//                         bgColor="#E6E2D8"
//                         textColor="#1A1A1A"
  ...
  const fetchOrders = async () => {
    try {
      ...
      const transformedOrders = response.data.map((order: any) => ({
        ...
        customer: order.user?.name || order.user?.email || 'Unknown',
        total: `৳${order.totalPrice || 0}`,
        status: order.status.charAt(0).toUpperCase() + order.status.slice(1).toLowerCase(), // ✅ Capitalize first letter
      }));
